...
from src.segformer.mit import mit_b0, mit_b1, mit_b2, mit_b3, mit_b4, mit_b5
from src.segformer_fine.T2 import T1, EnhancedSegFormerHead
...
class SegFormer_fine(nn.Module):
    def __init__(self, num_classes = 21, mit = 'b2', pretrained = False,t1 = True,t2 = True):
        super(SegFormer_fine, self).__init__()
        self.in_channels = {
        ...
        if t1:
            self.decode_head = EnhancedSegFormerHead(num_classes, self.in_channels, self.embedding_dim)
        else:
...
if __name__ == '__main__':
    ...
    model = SegFormer_fine(mit='b0').cuda()
